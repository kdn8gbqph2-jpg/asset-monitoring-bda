...
namespace asset_monitoring.Pages
{
    public class OperatorModel : AppPageModel
    {
        ...
        {
            if (!IsLoggedIn || (UserType != "OPERATOR" && UserType != "ADMIN")) return RedirectToPage("/Index");
            try
            {
                ...
                Logger.Info("Operator ExportRunningLog: {0} pumps, {1} events, period={2}-{3}, userId={4}",
            ...
            {
                Logger.Error(ex, "Operator ExportRunningLog failed");
